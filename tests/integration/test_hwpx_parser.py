...
import openhanji
from openhanji.document import Document, ImageRef, ParagraphStyle
...
class TestBasicDocument:
    ...
    def test_metadata_is_metadata_object(self, doc):
        from openhanji.document import Metadata
...
class TestErrorHandling:
    ...
    def _make_hwpx(self, tmp_path, name: str, section_xml: str) -> pathlib.Path:
        path = tmp_path / name
        with zipfile.ZipFile(path, "w") as zf:
    ...
    def test_text_box_gso_text_extracted(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "gso.hwpx", sec))
    ...
    def test_footnote_inlined(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "fn.hwpx", sec))
    ...
    def test_endnote_inlined(self, tmp_path):
        sec = self._sec(
            ...
            '<hp:endnote><hp:p><hp:run><hp:t>미주 내용</hp:t></hp:run></hp:p></hp:endnote>'
            '</hp:p>'
        )
        doc = openhanji.open(self._make_hwpx(tmp_path, "en.hwpx", sec))
    ...
    def test_equation_placeholder_extracted(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "eq.hwpx", sec))
        assert len(doc.paragraphs) == 1
    ...
    def test_nested_table_keeps_structured_blocks_and_recursive_text(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "nested.hwpx", sec))
    ...
    def test_nested_table_only_document_renders_and_does_not_warn(self, tmp_path, caplog):
        sec = self._sec(
            '<ht:tbl>'
            '<ht:tr>'
            '<ht:tc>'
        ...
        with caplog.at_level(logging.WARNING, logger="openhanji.parsers.hwpx"):
            doc = openhanji.open(self._make_hwpx(tmp_path, "nested_only.hwpx", sec))
    ...
    def test_inline_table_splits_paragraph_and_preserves_order(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "mixed.hwpx", sec))
    ...
    def test_equation_in_cell_survives_recursive_text_and_renderers(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "cell_eq.hwpx", sec))
    ...
    def test_simple_table_markdown_stays_gfm(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "simple_md.hwpx", sec))
        markdown = doc.to_markdown()
    ...
    def test_complex_table_markdown_uses_html_and_preserves_spans(self, tmp_path):
        ...
        doc = openhanji.open(self._make_hwpx(tmp_path, "complex_md.hwpx", sec))
        markdown = doc.to_markdown()
    ...
    def test_bindata_case_insensitive(self, tmp_path):
        png_bytes = (
            pathlib.Path(__file__).parent.parent / "test_files" / "png" / "inline-rgb-1x1.png"
        ).read_bytes()
        sec = self._sec(
            '<hp:p><hp:run>'
    ...
    def test_spine_order_overrides_filename_sort(self, tmp_path):
        ...
        section0 = self._sec('<hp:p><hp:run><hp:t>zero</hp:t></hp:run></hp:p>')
        section1 = self._sec('<hp:p><hp:run><hp:t>one</hp:t></hp:run></hp:p>')
        content = (
            '<?xml version="1.0" encoding="UTF-8"?>'
